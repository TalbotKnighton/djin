...
from djin.containers import default_registry

__all__ = [
    "ProductsOfInertiaSignConvention",
    "InertiaTensor",
    "CenterOfMass",
    "MassProperties",
]
...
class InertiaTensor(BaseModel):
    ...
    def tensor(self) -> Tensor3x3:
        ...
        c = (
            ...
            == ProductsOfInertiaSignConvention.positive_integrals
            else 1
        )
        return np.array(
            [
        ...
        **kwargs,
        ...
        if handler == InertiaErrorStrategy.RAISE_EXCEPTION:
            ...
            if error_type == InertiaErrorType.TRIANGLE_INEQUALITY_VIOLATION:
                ...
                raise ValueError(
                    f"Strategy {handler} not applicable for error type {error_type}"
                )

        elif handler == InertiaErrorStrategy.MAKE_POSITIVE_DEFINITE:
            if error_type == InertiaErrorType.NEGATIVE_EIGENVALUES:
                return self._make_positive_definite(
                    tensor, kwargs.get("eigenvalues"), kwargs.get("eigenvectors")
                )
            else:
                raise ValueError(
                    f"Strategy {handler} not applicable for error type {error_type}"
    ...
    def _make_positive_definite(
        tensor: np.ndarray, eigenvalues: np.ndarray, eigenvectors: np.ndarray
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def tensor_is_3x3(tensor: Tensor3x3) -> bool:
        """
    ...
    def tensor_is_symmetric(
        ...
        """
        return np.allclose(
            np.array(tensor), np.array(tensor).transpose(), rtol=0, atol=tolerance
        )

    @staticmethod
    def get_tensor_3x3_eigenvalues_sorted_asserted_real_positive_semidefinite(
        tensor: Tensor3x3, eigen_value_real_rel_tolerance: Number = 1e-12
    ):
        """
        ...
        try:
            assert all(
                (eig_vals_imag_abs / eig_vals_real_abs) < eigen_value_real_rel_tolerance
            )
    ...
    def tensor_3x3_obeys_triangle_inequality(
        cls, tensor: Tensor3x3, eigen_value_real_tolerance: Number = 1e-12
    ) -> bool:
        """
        ...
        """
        eig_vals = (
            cls.get_tensor_3x3_eigenvalues_sorted_asserted_real_positive_semidefinite(
                tensor=tensor, eigen_value_real_rel_tolerance=eigen_value_real_tolerance
            )
        )
    ...
    def from_tensor(
        ...
        cls.assert_inertia_tensor_is_physical(
            tensor=tensor, relative_tolerance=relative_symmetry_tolerance
        )
        c = (
            -1
            if sign_convention == ProductsOfInertiaSignConvention.positive_integrals
            else 1
        )
        return cls(
            i_xx=tensor[0, 0],
    ...
    def change_frame(self, old_frame: Frame, new_frame: Frame, mass: Number) -> Self:
        ...
        rotated_tensor = (
            f.pose.rotation.as_matrix()
            @ self.tensor
            @ f.pose.rotation.inv().as_matrix()
        )
            ...
            match self.sign_convention:
                case ProductsOfInertiaSignConvention.positive_integrals:
                    self.sign_convention = (
                        ProductsOfInertiaSignConvention.negative_integrals
                    )
...
class CenterOfMass(BaseModel):
    ...
    def __rsub__(self, other: CenterOfMass):
        ...
        new_position = (
            other.position * other.mass - self.position * self.mass
        ) / total_mass
        return self._constructor(mass=total_mass, position=new_position)
...
class MassProperties(BaseModel):
    ...
    def change_frame(self, new_frame: Frame) -> Type[Self]:
        ...
        """
        new_center_of_mass = self.center_of_mass.change_frame(new_frame=new_frame)
        new_inertia_tensor = self.inertia_tensor.change_frame(
            old_frame=self.frame, new_frame=new_frame, mass=self.center_of_mass.mass
        )
        _constructor = type(self)
        return _constructor(
...
def main():
    ...
    cma = CenterOfMass(
        mass=5.0, position=Point3D.from_components(x=1, y=0, z=0, frame=fa)
    )
    cmb = CenterOfMass(
        mass=5.0, position=Point3D.from_components(x=1.0, y=0, z=0, frame=fb)
    )
    cmc = cma + cmb
    print(cma + cmb)
